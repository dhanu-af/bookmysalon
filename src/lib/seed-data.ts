...
/**
 * Idempotent (upsert / find-or-create throughout) — safe to run repeatedly
 * against the same database. Shared by the local CLI seed script
 * (prisma/seed.ts) and the one-time production seed route
 * (src/app/api/admin/seed/route.ts), since the CLI script can't reach a
 * database it doesn't have a local connection string for.
 */
export async function seedDemoData(prisma: PrismaClient) {
  ...
  for (const s of SALONS) {
    ...
    if (salon.slug === "brisbane-barber-co") {
      ...
      const haircut = services.find((svc) => svc.name === "Haircut")!;
